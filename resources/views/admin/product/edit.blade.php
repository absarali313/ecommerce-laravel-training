                ...
                <div class="mb-3 row">
                    @foreach($product->images as $productImage)
                        <div class="col-3 mx-2 flex justify-content-center">
                            <form id="imageForm" method="POST" action='{{ route("products.images.delete", $productImage->id) }}'>
                                @csrf
                                @method('DELETE')
                                <img src="{{asset($productImage->image_path)}}"  class="rounded-2 m-1 border border-1 border-white" alt='product' style='height:150px; width:150px'>
                ...
                <div class="col">
                    <form method="POST" action="{{route("products.size.store",$product)}}">
                        ...
                        <div class="col">
                            ...
                            <input id="size_title" name="size_title"  class="bg-white-50 border border-opacity-25 border-black rounded-2 px-2 w-25"  placeholder="Small">
                            <x-form-error name="size_title"></x-form-error>

                            <label for="price" class="mx-2 text-start text-secondary">Price</label>
                            <input id="price" name="price" type="number"  class="bg-white-50 border border-opacity-25 border-black rounded-2 px-2 w-25"  placeholder="Rs. 1000">
                            <x-form-error name="price"></x-form-error>

                            <label for="stock" class="mx-2 text-start text-secondary">Stock</label>
                            <input id="stock" name="stock" type="number"  class="bg-white-50 border border-opacity-25 border-black rounded-2 px-2 "  placeholder="Small">
                            <x-form-error name="price"></x-form-error>
                ...
                <div class="col">
                    <form method="POST" action="{{route('products.related.store',$product)}}">
                        ...
                        <div class="col">
                            ...
                            <input id="product_id" name="product_id" type="number" class="bg-white-50 border border-opacity-25 border-black rounded-2 px-2 w-25" placeholder="{{$product->id}}">
